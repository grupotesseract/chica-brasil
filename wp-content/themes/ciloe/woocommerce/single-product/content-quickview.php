...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>"
    ...
    <figure class="woocommerce-product-gallery__wrapper nav-caret">
		...
		if ( has_post_thumbnail() ) {
			$ciloe_img_size = ciloe_resize_image( get_post_thumbnail_id( $post->ID ), null, 553, 670, true, true, false );
			...
			$html           .= ciloe_img_output( $ciloe_img_size, 'wp-post-image' );
			$html           .= '</a></div>';
		} else {
			$html = '<div class="woocommerce-product-gallery__image--placeholder">';
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_attr__( 'Awaiting product image', 'ciloe' ) );
			$html .= '</div>';
		}
		...
		if ( $attachment_ids && has_post_thumbnail() ) {
			foreach ( $attachment_ids as $attachment_id ) {
				...
				$ciloe_img_size  = ciloe_resize_image( $attachment_id, null, 553, 670, true, true, false );
				$html            = '<div data-thumb="' . esc_url( $thumbnail[0] ) . '" class="woocommerce-product-gallery__image"><a href="javascript:void(0)">';
				$html            .= ciloe_img_output( $ciloe_img_size );
				$html            .= '</a></div>';
